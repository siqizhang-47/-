...
def make_preds(tmp_path, model, n=6, H=24, S=10, seed=0, shard_size=4):
...
def test_alignment_mismatch_detected(tmp_path):
    ...
    w.write(np.abs(rng.normal(size=(6, 24, D, 10))).astype(np.float32),
            np.abs(rng.normal(size=(6, 24, D))).astype(np.float32),
