...
def rectified_impute(X0, M, Xstar, modelclass, max_rounds = 100, clamp = False, callback = None, verbose=False,
                    ...
                    lr = 0.01, estimate_mi = True, checkpoint_path = None):
    d = X0.shape[1] 
    n = X0.shape[0]
    ...
    try:
        for round in range(max_rounds):
            ...
            # save checkpoint (only when a path is provided; keeps IES sweeps from
            # writing/overwriting checkpoints in the working directory)
            if checkpoint_path is not None:
                torch.save([X0, mmd_list, mi_list, model_list], checkpoint_path)
