...
# Same architecture as V2 fair run, with a larger training budget only.
...
python3 -u ./src/experiments/NsDiff_energy_exog.py \
  ...
  --model_type=NsDiffMeanVarV2ExogTuned \
  ...
  --load_pretrain=False \
  --analysis_dir=./energy4_exog_outputs/mean_and_var_v2_tuned \
